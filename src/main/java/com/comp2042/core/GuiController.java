...
/**
 * The main controller for the JavaFX GUI.
 * Manages the application's user interface, including the main menu, game
 * board,
 * settings, and game over screens. It handles user input mapping to game
 * actions,
 * updates the visual state of the game (rendering bricks, score, next pieces),
 * and coordinates transitions between different UI panels.
 *
 * Implements {@link Initializable} to set up UI components after FXML loading.
 */
public class GuiController implements Initializable {
     ...
     * It sets up the game board dimensions, loads custom fonts, initializes
     * system managers (audio, settings, high scores), and configures initial
     * UI states.
     *
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        javafx.application.Platform.runLater(this::applyCustomFontsToAllLabels);
    ...
    /**
     * Applies the loaded custom font to all Label components in the UI.
     * This ensures a consistent visual style across the score, level, and
     * high score displays. It differentiates between large fonts for main
     * indicators and medium fonts for secondary labels.
     */
    private void applyCustomFontsToAllLabels() {
        ...
        try {
            ...
            // Apply fonts to header labels in info boxes
    ...
    /**
     * Recursively applies a specified font to all Label children within a parent
     * node.
     * Only affects labels with the "info-header-text" style class.
     *
    ...
    /**
     * Loads custom TrueType fonts from the resources folder.
     * Attempts to load a regular and a bold pixel-style font ("BoutiqueBitmap9x9").
     * If loading fails, it falls back to a standard Arial font to ensure text
     * remains readable.
     */
    ...
    /**
     * Calculates the size of each brick cell dynamically based on the screen
     * height.
     * This ensures the game board fits comfortably on various screen resolutions,
     * maintaining a target height of approximately 85% of the screen.
     * The size is clamped between 25px and 40px to prevent display issues.
     */
    ...
    /**
     * Converts a logical game row index to its corresponding display row index.
     * The game logic uses a 22-row grid where the top 2 rows are a hidden buffer.
     * This method shifts the index so row 2 in logic becomes row 0 in display.
     *
     * @param gameRow The row index in the game logic matrix (0-21).
     * @return The row index for the UI grid (0-19).
     */
    ...
    /**
     * Configures the layout dimensions of the game board and related panels.
     * Sets the width and height of the main game panel, brick panel, ghost panel,
     * and background borders based on the calculated {@code BRICK_SIZE}.
     * Also aligns the "Next Piece" container.
     */
    private void setupBoardDimensions() {
        ...
        // Ensure boardContainer aligns to top for proper alignment with side
        // boxes
    ...
    /**
     * Sets up keyboard event handlers for the game loop.
     * Maps user key presses to game actions (Move Left/Right, Rotate, Soft/Hard
     * Drop, Hold, Pause).
     * Uses the {@link KeyBindings} manager to check for custom key configurations.
     * Interactions are only processed if the game is not paused and not over.
     */
     ...
     * Plays the start game sound effect and triggers a transition from the
     * main menu to the active game view. It delegates the logical game reset
     * to {@link #newGame(ActionEvent)}.
     */
